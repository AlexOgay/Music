"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.GenreController = void 0;
const common_1 = require("@nestjs/common");
const create_genre_dto_1 = require("../dto/create-genre-dto");
const update_track_genre_dto_1 = require("../dto/update-track_genre-dto");
const track_service_1 = require("../track/track.service");
const genre_service_1 = require("./genre.service");
let GenreController = class GenreController {
    constructor(genreservice, trackservice) {
        this.genreservice = genreservice;
        this.trackservice = trackservice;
    }
    async create(genre) {
        const track = await this.trackservice.getone(genre.track_id);
        return await this.genreservice.creategenre(genre, track);
    }
    async getall() {
        return this.genreservice.getall();
    }
    async getone(id) {
        return await this.genreservice.getone(id);
    }
    async creategenre_fortrack(data) {
        const track = await this.trackservice.getone(data.track_id);
        const genre = await this.genreservice.getone(data.genre_id);
        return await this.genreservice.createfortrack(track, genre);
    }
};
__decorate([
    (0, common_1.Post)(),
    __param(0, (0, common_1.Body)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [create_genre_dto_1.CreateGenreDto]),
    __metadata("design:returntype", Promise)
], GenreController.prototype, "create", null);
__decorate([
    (0, common_1.Get)(),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", []),
    __metadata("design:returntype", Promise)
], GenreController.prototype, "getall", null);
__decorate([
    (0, common_1.Get)('/:id'),
    __param(0, (0, common_1.Param)('id')),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Object]),
    __metadata("design:returntype", Promise)
], GenreController.prototype, "getone", null);
__decorate([
    (0, common_1.Post)('/track'),
    __param(0, (0, common_1.Body)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [update_track_genre_dto_1.UpdateTrackGenreDto]),
    __metadata("design:returntype", Promise)
], GenreController.prototype, "creategenre_fortrack", null);
GenreController = __decorate([
    (0, common_1.Controller)('/genre'),
    __metadata("design:paramtypes", [genre_service_1.GenreService,
        track_service_1.TrackService])
], GenreController);
exports.GenreController = GenreController;
//# sourceMappingURL=genre.controller.js.map